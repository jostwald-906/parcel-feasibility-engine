...
async def test_bergamot_btv_integration():
    ...
    bergamot_scenarios = [
        s for s in response.alternative_scenarios if "Bergamot" in s.scenario_name
    ]
    print(f"  - Bergamot scenarios: {len(bergamot_scenarios)}")

    for scenario in bergamot_scenarios:
        print(
            f"    • {scenario.scenario_name}: {scenario.max_units} units, FAR {scenario.max_building_sqft / parcel.lot_size_sqft:.2f}"
        )

    # Verify Bergamot scenarios were generated
    assert (
        len(bergamot_scenarios) == 3
    ), f"Expected 3 Bergamot scenarios, got {len(bergamot_scenarios)}"
...
async def test_bergamot_cac_small_integration():
    ...
    assert (
        tier_3.max_height_ft == 86
    ), f"Expected Tier 3 height 86 ft for small CAC, got {tier_3.max_height_ft}"
...
async def test_non_bergamot_parcel():
    ...
    assert (
        len(bergamot_scenarios) == 0
    ), f"Expected 0 Bergamot scenarios for R2 parcel, got {len(bergamot_scenarios)}"
