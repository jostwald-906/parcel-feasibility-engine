...
class AffordableHousing(BaseModel):
    ...
    affordability_duration_years: int = Field(
        55, ge=1, description="Affordability duration in years"
    )
...
class ProposedProject(BaseModel):
    ...
    average_bedrooms_per_unit: Optional[float] = Field(
        None, ge=0, description="Average bedrooms per unit"
    )
    ...
    average_unit_size_sqft: Optional[float] = Field(
        None, ge=0, description="Average unit size in sqft"
    )
    total_building_sqft: Optional[float] = Field(
        None, ge=0, description="Total building square footage"
    )
    ...
    # Affordable Housing
    affordable_housing: Optional[AffordableHousing] = Field(
        None, description="Affordable housing plan"
    )
    ...
    # Site Configuration
    site_configuration: Optional[SiteConfiguration] = Field(
        None, description="Site layout configuration"
    )
