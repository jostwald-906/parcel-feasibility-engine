...
class RentControlData(BaseModel):
    ...
    is_rent_controlled: bool = Field(
        False, description="Whether property has rent-controlled units"
    )
    total_units: int = Field(0, description="Total number of units with MAR data")
    avg_mar: Optional[float] = Field(
        None, description="Average MAR across all units (excludes $0 exempt units)"
    )
    units: List[RentControlUnit] = Field(
        default_factory=list, description="Individual unit details"
    )
...
class AnalysisRequest(BaseModel):
    ...
    proposed_project: Optional[ProposedProject] = Field(
        None, description="Proposed project details for validation against allowed development"
    )
    ...
    target_affordability_pct: Optional[float] = Field(
        None, ge=0, le=100, description="Target percentage of affordable units"
    )
...
class DevelopmentScenario(BaseModel):
    ...
    setbacks: Dict[str, float] = Field(
        default_factory=dict, description="Setback requirements (front, rear, side)"
    )
    lot_coverage_pct: float = Field(
        ..., ge=0, le=100, description="Maximum lot coverage percentage"
    )

    # Financial estimates (optional)
    estimated_buildable_sqft: Optional[float] = Field(
        None, description="Estimated buildable square footage"
    )
    notes: List[str] = Field(
        default_factory=list, description="Additional notes and considerations"
    )

    # Density bonus specific fields (optional)
    concessions_applied: Optional[List[str]] = Field(
        None, description="Concessions applied (§ 65915(d))"
    )
    waivers_applied: Optional[List[str]] = Field(None, description="Waivers applied (§ 65915(e))")

    # Timeline estimation (optional)
    estimated_timeline: Optional[Dict[str, Any]] = Field(
        None, description="Entitlement timeline estimation"
    )
...
class AnalysisResponse(BaseModel):
    ...
    alternative_scenarios: List[DevelopmentScenario] = Field(
        default_factory=list, description="Alternative development scenarios (SB9, SB35, etc.)"
    )
    ...
    potential_incentives: List[str] = Field(
        default_factory=list, description="Potential incentives available"
    )
    ...
    rent_control: Optional[RentControlData] = Field(
        None, description="Rent control data if available"
    )

    # CNEL (noise) analysis
    cnel_analysis: Optional[Dict[str, Any]] = Field(
        None, description="Community Noise Equivalent Level analysis"
    )

    # Community benefits analysis
    community_benefits: Optional[Dict[str, Any]] = Field(
        None, description="Available community benefits for tier upgrades"
    )

    # Proposed project validation (if proposed_project was provided in request)
    proposed_validation: Optional[Dict[str, Any]] = Field(
        None, description="Validation results comparing proposed project against allowed scenarios"
    )

    # Debug information (only included when debug=True)
    debug: Optional[Dict[str, Any]] = Field(
        None, description="Debug information with decision trace"
    )
